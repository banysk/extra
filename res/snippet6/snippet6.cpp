...
#include "cpp/Texture.cpp"
 ...
 int main() {
	...
	// шейдеры и текстуры
	Shader box("res/shaders/spot.vs", "res/shaders/spot.fs");
	...
	while (isGo) {
        ...
        //glm::vec3 light_move = glm::vec3(x, y, z);

        //lamp.set_mat4("projection", Camera.get_projection());
        //lamp.set_mat4("view", Camera.get_view());
        //lamp.set_mat4(
        //    "model",
        //   glm::scale(glm::translate(Camera.get_model(), lightPos), glm::vec3(0.2f))
        //);
        ...
        box.set_vec3("light.position", Camera.get_pos());
        box.set_vec3("light.direction", Camera.get_front());
        box.set_float("light.cutOff", glm::cos(glm::radians(12.5f)));
        box.set_vec3("viewPos", Camera.get_pos());

        box.set_vec3("light.ambient", 0.1f, 0.1f, 0.1f);

        box.set_vec3("light.diffuse", 0.8f, 0.8f, 0.8f);
        box.set_vec3("light.specular", 1.0f, 1.0f, 1.0f);
        box.set_float("light.constant", 1.0f);
        box.set_float("light.linear", 0.09f);
        box.set_float("light.quadratic", 0.032f);
