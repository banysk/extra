...
#include "cpp/Texture.cpp"
 ...
 int main() {
	...
	// шейдеры и текстуры
	Shader box("res/shaders/directional.vs", "res/shaders/directional.fs");
	...
	while (isGo) {
        ...
        box.set_vec3("light.direction", -0.2f, -1.0f, -0.3f);

        diffuseMap.bind(0);
        specularMap.bind(1);

        glm::vec3 lightColor;
        lightColor.x = sin(phi * 2.0f);
        lightColor.y = sin(phi * 0.7f);
        lightColor.z = sin(phi * 1.3f);
        ...
        box.set_vec3("light.ambient", ambientColor);
        box.set_vec3("light.diffuse", diffuseColor);
        box.set_vec3("light.specular", 1.0f, 1.0f, 1.0f);

        box.set_vec3("material.specular", 0.5f, 0.5f, 0.5f);
        box.set_float("material.shininess", 64.0f);
